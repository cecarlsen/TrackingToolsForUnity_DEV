/*
	...
	Note: Perhaps a more alternative solution by DTU students.
...
namespace TrackingTools
{
	public class ProjectorFromCameraExtrinsicsEstimator : MonoBehaviour
	{
		...
		[SerializeField,ReadOnlyAtRuntime,Tooltip( "Number of inner corners" )] Vector2Int _chessPatternSize = new Vector2Int( 5, 8 );
		[SerializeField,ReadOnlyAtRuntime,Tooltip( "Millimeters" )] int _chessTileSize = 40; // mm.
		[SerializeField,ReadOnlyAtRuntime,Tooltip( "Millimeters" )] int _chessCirclePatternCenterOffset = 420; // mm
		[SerializeField,ReadOnlyAtRuntime] bool _flipCameraTexture = false;
		...
		CameraExtrinsicsCalibrator _cameraExtrinsicsCalibrator;
		...
		void Update()
		{
			if( !_cameraTexture || !_dirtyTexture ) return;
		...
		void UpdateCirclePatternSize()
		{
			...
			if( _state == State.TrackedCalibration || _state == State.Testing ) _circlePatternTransform.localPosition = -Vector3.right * ( _chessCirclePatternCenterOffset  / 1000f );
		}
		...
		void SaveToFiles()
		{
			...
			if( _addErrorValueToFileName ) intrinsicsFileName += "_E-" + _projectorIntrinsicsCalibrator.rmsError.ToString( "F02" ).Replace( ".", "," );
		...
		void OnDrawGizmos()
		{
			if( _processedCameraTexture == null || _circlePointsRenderedWorldMat == null ) return;
